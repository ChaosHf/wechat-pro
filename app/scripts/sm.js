...
(function () {
    ...
    FastClick.prototype.onTouchStart = function(event) {
        ...
        // 如果是select元素 禁用fastclick直接返回false
...
(function($) {
     ...
     * @param isPushState
       */
    Router.prototype.loadDirection = function(url, direction, isPushState) {
      if (this._isTheSameDocument(location.href, url)) {
        this._switchToSection(Util.getUrlFragment(url), direction, isPushState);
      } else {
        this._saveDocumentIntoCache($(document), location.href);
        this._switchToDocument(url, isPushState, direction);
      }
    };
    ...
    Router.prototype._doSwitchDocument = function(url, isPushState, direction) {
        ...
        // 从cache中获取document的title
        var $newDocTitle = ''; //this.cache[urlObj.base].$title; //暂时不使用
        ...
        this._animateDocument($currentDoc, $newDoc, $visibleSection, direction, $newDocTitle);
    ...
    /**
     * 将string转为DOM元素，该方法有问题，暂时不使用
     *
     * 注意：string的html的head中不能包含script标签，script标签需要放到body中
     *
     * @param data
     * @returns {Document}
     * @private
     */
    Router.prototype._str2DOMElement = function(data) {
      // 创建临时的iframe以获取完整的document对象
      ...
      var el = frame.contentDocument.documentElement;
      // 删除临时创建的iframe
    ...
    Router.prototype._loadDocument = function(url, callback) {
        ...
        this.xhr = $.ajax({
            ...
            success: $.proxy(function(data, status, xhr) {
                ...
                var $doc = $('<html></html>');
                $doc.append(data);
                // 采用string转DOM的方式将data转换为DOM对象，原有的转换方式不能修改document.title
                // var $doc = $(this._str2DOMElement(data)); //暂时不使用，有问题
                if(callback.success)
     ...
     * @private
     */
    Router.prototype._animateDocument = function($from, $to, $visibleSection, direction, $toTitle) {
        ...
        $to.animationEnd(function() {
            ...
            // 暂时不修改document.title，浏览器会支持，但微信等应用似乎没有监听document.title的改变
    ...
    $(function() {
        ...
        $(document).on('click', 'a', function(e) {
            ...
            if ($target.hasClass('back')) {
                ...
                // 根据class判断页面载入方向
                if ($target.hasClass('from-left-to-right')) {
                  router.loadDirection(url, DIRECTION.leftToRight);
                } else if ($target.hasClass('from-right-to-left')) {
                  router.loadDirection(url, DIRECTION.rightToLeft);
                } else {
                  router.load(url);
                }
